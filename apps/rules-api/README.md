...
- `_id` is generated by the API as a UUID when a rule is created.
...
On success, `_id` is generated as a UUID and `createdAt` and `modifiedAt` are set to the current UTC time. A client-provided `_id` is ignored. Duplicate sensor values are normalized.
