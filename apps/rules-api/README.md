...
- `minResolution` and `maxResolution` must be greater than `0`.
- `maxResolution` must be greater than or equal to `minResolution` and defaults to `999`.
...
On success, Elasticsearch generates `_id`, and `createdAt` and `modifiedAt` are set to the current UTC time. A client-provided `_id` is ignored. Duplicate sensor values are normalized.
...
- `maxResolution` cannot be set to `null`, `0`, or a negative value.
